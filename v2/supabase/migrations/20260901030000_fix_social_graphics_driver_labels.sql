-- Keep Social Graphics driver labels stable across every current and future league.
-- Access remains restricted to authenticated league administrators by capability.
...
begin
  ...
  return coalesce((
    select jsonb_agg(
      jsonb_build_object(
        'driverId', driver.id,
        'leagueDriverName', driver.display_name,
        'driverName', nullif(btrim(identity_row.real_name), ''),
...
comment on function public.get_social_graphics_driver_labels() is
  'Returns league-scoped real-name, display-name and gamertag labels for authorized Social Graphics editors.';
