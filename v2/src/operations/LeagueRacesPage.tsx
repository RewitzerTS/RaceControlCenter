...
export function LeagueRacesPage() {
  ...
  const stewardRevisions = workspace.races.filter((race) => race.result_revision?.stewardCaseNumber);
  const currentSeason = workspace.seasons.find((season) => season.is_active);
  const completionBlockers = currentSeason ? seasonCompletionBlockers(workspace.races, currentSeason.id) : [];
