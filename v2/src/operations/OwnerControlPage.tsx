...
export function OwnerControlPage() {
  ...
  return <main className="operations-page" id="main-content">
    ...
    <div className="owner-grid">
      <section className="owner-leagues"><h2>{t('owner.leagues')}</h2><div className="responsive-table" role="region" tabIndex={0}><table><thead><tr><th>{t('owner.league')}</th><th>{t('owner.status')}</th><th>{t('owner.action')}</th></tr></thead><tbody>{snapshot.leagues.map((league) => <tr key={league.id}><td><strong>{league.name}</strong><small>{league.slug}</small></td><td>{league.status}</td><td><button type="button" onClick={() => { setLeagueSlug(league.slug); navigate('/admin'); }}>{t('owner.open')}</button></td></tr>)}</tbody></table></div></section>
