...
const ADMIN_AREAS: Array<{ title: MessageKey; items: MessageKey[] }> = [
  { title: 'admin.raceOps', items: ['admin.races', 'admin.results', 'admin.import', 'admin.standings', 'admin.championship'] },
  { title: 'admin.participants', items: ['admin.drivers', 'admin.teams', 'admin.users'] },
  { title: 'admin.stewarding', items: ['admin.cases', 'admin.rules'] },
  { title: 'admin.content', items: ['admin.graphics', 'admin.publishing'] },
  { title: 'admin.league', items: ['admin.settings', 'admin.branding', 'admin.permissions', 'admin.audit'] },
];
...
export function AdminWorkspacePage() {
  ...
  return <main className="operations-page" id="main-content">
    ...
    <div className="operations-layout">
      <nav className="operations-menu" aria-label={t('admin.navigation')}>{ADMIN_AREAS.map((area) => <section key={area.title}><h2>{t(area.title)}</h2>{area.items.map((item) => <button key={item} type="button">{t(item)}<span aria-hidden="true">→</span></button>)}</section>)}</nav>
