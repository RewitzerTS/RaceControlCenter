        ...
        <table className="result-review-table">
          ...
          <tbody>{rows.map((row, index) => {
            ...
            return <tr className={row.driverId ? '' : 'result-review-row--unmatched'} key={row.key}>
              ...
              <td><select aria-label={`Fahrer Zeile ${index + 1}`} onChange={(event) => selectDriver(row, event.target.value)} value={row.driverId}><option value="">Fahrer zuordnen</option>{drivers.filter((driver) => driver.is_active).map((driver) => <option key={driver.id} value={driver.id}>{driver.display_name}{driver.gamertag ? ` · ${driver.gamertag}` : ''}</option>)}</select><small>Erkannt: {row.rawDriver}</small></td>
