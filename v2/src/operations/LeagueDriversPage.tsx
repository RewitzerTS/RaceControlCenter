...
export function LeagueDriversPage() {
  ...
  return <main className="operations-page admin-management-page" id="main-content">
    ...
    {editing?.id && workspace?.active_season && <p><a className="text-link" href="#roster-title">{rosterCopies[language].profileHint}</a></p>}
    {workspace && <RosterWorkflowPanel drivers={workspace} onSaved={async () => { await reload(); setEditing(null); }} />}
