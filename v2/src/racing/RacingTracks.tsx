...
export function RacingTracks({ profile = false }: { profile?: boolean }) {
  ...
  return <ProfileFrame view={view} title={profile ? h.trackProfile : h.tracks} kind={profile ? 'track-profile' : 'tracks'}>{data && <>
    ...
    {!validSeason || (profile && !stats && tracks.length > 0) ? <p role="status">{c.missing}</p> : !tracks.length ? <p role="status">{h.emptyTracks}</p> : !profile ? <>
      ...
      <div className="profile-identity"><TrackMap meta={stats.meta} /><div className="track-identity-copy"><div className="profile-meta"><TrackFlag country={stats.meta.country} /><span>{info?.country || stats.meta.country}</span></div><h2>{stats.meta.name}</h2><p>{stats.meta.circuit}</p><p>{[info?.trackType, info?.direction].filter(Boolean).join(' · ')}</p></div><aside className="track-identity-record" aria-label="RaceVora Record"><span>RaceVora Record</span>{stats.bestLap ? <><strong className="profile-fastest">{stats.bestLap.text}</strong><HistoryDriverLink view={view} id={stats.bestLap.driverId} name={stats.bestLap.name} /></> : <p>{c.empty}</p>}</aside></div>
      <StatList values={[[c.race, n(stats.races)], [c.starts, n(stats.starts)], [c.driver, n(stats.uniqueDrivers)], [h.corners, info?.corners ?? '—'], [h.length, info?.lengthKm || '—'], [h.f1Record, <span className="track-f1-record">{info?.lapRecord || '—'}</span>]]} />
