...
requireGate(manifest.schemaVersion === 3, 'post-cutover recovery manifest schema is pinned');
...
requireGate(/^[0-9a-f]{40}$/.test(manifest.v2ProductionRelease.releaseCommit) && manifest.v2ProductionRelease.commitUrl.endsWith(manifest.v2ProductionRelease.releaseCommit), 'V2 Production release is pinned to an immutable commit');
...
requireGate(manifest.cutoverGates.phase29Authorized === true && manifest.cutoverGates.phase30Authorized === true && manifest.cutoverGates.v1ShutdownAllowed === false, 'authorized cutover retains a denied V1 shutdown');
requireGate(manifest.phaseProgress.phase29TrafficStatus === 'v2-active' && manifest.phaseProgress.phase30RetirementStatus === 'reversible-operational-retirement-complete', 'V2 traffic is active and V1 retirement remains reversible');
...
requireGate(manifest.phase30Preservation.ownerApprovalReceived === true && manifest.phase30Preservation.preservationStatus === 'complete-v1-retained-for-rollback', 'Phase 30 owner approval and completed preservation are recorded');
requireGate(manifest.phase30Preservation.v1DeletionAllowed === false && manifest.phase30Preservation.v1PauseAllowed === false && manifest.phase30Preservation.recoveryBranchRequired === true && manifest.phase30Preservation.productionObservationWindowComplete === true, 'Phase 30 preserves recoverable V1 after Production observation');
...
requireGate(phase2930Gate.includes('phase29Authorized === true') && phase2930Gate.includes('v1ShutdownAllowed === false'), 'Phase 29/30 code records cutover authorization while denying V1 shutdown');
...
requireGate(/^Status: complete\. V2 production traffic: active\./m.test(phase29), 'Phase 29 records active V2 Production traffic');
requireGate(/^Status: complete\. V1 operational retirement: reversible\./m.test(phase30), 'Phase 30 records reversible operational retirement');
requireGate(phase30.includes('not a shutdown'), 'V1 retirement preserves the recovery surface');
