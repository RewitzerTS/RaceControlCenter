(() => {
  ...
  async function initialize(options = {}) {
    ...
    if (!options.forceRefresh && state.league && state.initializedSlug === slug) {
      return snapshot();
    }
    if (!options.forceRefresh && state.initPromise && state.initializedSlug === slug) {
  ...
  function getLeagueId() {
    return state.league?.id || null;
  }

  function getSlug() {
    return state.league?.slug || state.initializedSlug || getRequestedLeagueSlug();
  }

  function getRole() {
    return state.membership?.role || null;
  }
