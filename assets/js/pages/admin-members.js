(() => {
  ...
  function bindReliableLogout() {
    const buttons = [
      document.getElementById('admin-banner-logout-btn'),
      document.getElementById('admin-quick-logout-btn')
    ].filter(Boolean);
    buttons.forEach((button) => {
      ...
      button.addEventListener('click', async (event) => {
        ...
        try {
          ...
          try {
            window.sessionStorage?.removeItem('rcc.activeLeagueSlug.v1');
            window.sessionStorage?.removeItem('rcc.lastTenantSlug.v1');
          } catch (_error) {}
          window.location.reload();
        } catch (error) {
          console.error('Logout fehlgeschlagen.', error);
          button.disabled = false;
          window.alert(`Logout fehlgeschlagen: ${error.message || 'Unbekannter Fehler'}`);
        }
      }, true);
    });
  }
  ...
  async function ensurePanel(platformOwner) {
    ...
    panel.innerHTML=`<summary><strong>Liga-Team & Zugänge</strong></summary><section class="panel admin-panel-wide admin-panel-accent">
      <h3>Mitglieder & Rollen</h3>
      <div class="notice">Die Ligaleitung kann Member und weitere Ligaleitungen verwalten. Die Owner-Rolle ist geschützt.</div>
      ...
      <div class="card-actions"><button type="button" class="button-primary" id="league-member-add-btn">Person hinzufügen / einladen</button><button type="button" class="button-secondary" id="league-members-refresh-btn">Liste aktualisieren</button></div>
      <div id="league-members-feedback" class="notice" hidden></div><div class="section-spacer-top"><h4>Aktuelle Mitglieder</h4><div id="league-members-list" class="stack-list"><div class="notice">Mitglieder werden geladen...</div></div></div>
    </section>`; layout.appendChild(panel);
