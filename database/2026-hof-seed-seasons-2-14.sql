-- Voraussetzung: Deine alten Saisons heißen in public.seasons bereits 2 bis 14.
-- Falls einige Saisonnamen anders heißen, passe die Werte im CTE unten an.
...
with source(season_name, driver_champion, driver_champion_team, constructor_champion, constructor_champion_lineup) as (
  ...
  select
    s.id as season_id,
    source.season_name,
    source.driver_champion,
    source.driver_champion_team,
    source.constructor_champion,
    source.constructor_champion_lineup
  from source
  join public.seasons s
    on s.name = source.season_name
)
