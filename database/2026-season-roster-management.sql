...
declare
  ...
  v_existing_slot_assignment_id uuid;
begin
  if v_user_id is null then raise exception 'Authentication required'; end if;
  if lower(coalesce(p_role, '')) not in ('primary','reserve','inactive') then
    raise exception 'Role must be primary, reserve or inactive';
  end if;
  ...
  if v_driver_league_id is null or v_driver_league_id <> v_league_id then
    raise exception 'Driver does not belong to this league';
  end if;

  if p_effective_from_race_id is not null then
    select r.round_number into v_round from public.races r
    where r.id = p_effective_from_race_id and r.season_id = p_season_id;
    if v_round is null then raise exception 'Effective race does not belong to this season'; end if;
  else
    select coalesce(max(r.round_number), 0) + 1 into v_round
    from public.races r
    where r.season_id = p_season_id and r.status in ('completed','official');
    v_round := greatest(coalesce(v_round, 1), 1);
  end if;

  if lower(p_role) = 'primary' then
    if p_team_id is null then raise exception 'Primary driver requires a team'; end if;
    select t.league_id, t.display_name into v_team_league_id, v_team_name
    ...
    where sds.season_id = p_season_id
      and lower(sts.slot_label) = lower(v_team_name)
      and not exists (
  ...
  -- Close the driver's currently active fixed-seat assignment immediately before the change.
  update public.driver_slot_assignments
  set valid_until_round_number = v_round - 1,
      valid_until_race_id = null
  where season_id = p_season_id
    and participant_driver_id = p_driver_id
  ...
  insert into public.driver_season_assignments (
    season_id, driver_id, team_id, league_team, is_primary,
    effective_from_race_id, effective_round_number
  ) values (
    p_season_id,
    p_driver_id,
    case when lower(p_role) = 'inactive' then null else p_team_id end,
    case when lower(p_role) = 'inactive' then null else v_team_name end,
    lower(p_role) = 'primary',
  ...
  if lower(p_role) = 'primary' then
    insert into public.driver_slot_assignments (
      ...
      v_driver_slot_id, p_season_id, p_driver_id, p_driver_id,
      'player', p_effective_from_race_id, v_round
    ) returning id into v_existing_slot_assignment_id;
  end if;
  ...
  return jsonb_build_object(
    ...
    'role', lower(p_role),
    'team_id', case when lower(p_role) = 'inactive' then null else p_team_id end,
    'effective_round_number', v_round,
    'slot_assignment_id', v_existing_slot_assignment_id
  );
end;
$$;

revoke execute on function public.set_season_driver_status(uuid, uuid, text, uuid, uuid) from public, anon;
grant execute on function public.set_season_driver_status(uuid, uuid, text, uuid, uuid) to authenticated;
