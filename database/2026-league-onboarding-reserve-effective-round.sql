-- Follow-up to the season-slot onboarding migration.
-- Reserve drivers are registered as available from round 1 so they satisfy the
-- existing driver_season_assignments_effective_source_check constraint.
...
begin
  ...
  loop
    ...
    insert into public.driver_season_assignments (
      season_id, driver_id, team_id, league_team, is_primary, effective_round_number
    )
    values (
      v_season_id, v_driver_id, v_team_id, nullif(v_team_name, ''),
      v_driver_role = 'primary',
      case when v_driver_role = 'reserve' then 1 else null end
    );
    ...
    if v_driver_role = 'primary' then
      ...
      insert into public.driver_slot_assignments (
        ...
        participation_mode, effective_round_number
      )
  ...
  return jsonb_build_object(
    'ok', true, 'league_id', p_league_id, 'season_id', v_season_id,
    ...
    'teams_created', v_team_count, 'races_created', v_round
  );
end;
$$;
