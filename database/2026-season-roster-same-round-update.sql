-- Treat a roster change at the same effective race/round as an edit instead of a duplicate assignment.
...
as $$
declare
  v_user_id uuid := auth.uid(); v_league_id uuid; v_member_role text; v_driver_league_id uuid; v_team_league_id uuid; v_team_name text; v_round integer; v_driver_slot_id uuid; v_slot_assignment_id uuid; v_last_completed_round integer; v_updated integer := 0;
begin
  ...
  select coalesce(max(r.round_number),0) into v_last_completed_round from public.races r where r.season_id=p_season_id and r.status in ('completed','official');
  ...
  else v_round := greatest(v_last_completed_round+1,1); end if;
  ...
  if lower(p_role)='primary' and p_team_id is null then raise exception 'Primary driver requires a team'; end if;

  delete from public.driver_slot_assignments
  where season_id=p_season_id and participant_driver_id=p_driver_id
    and coalesce(effective_round_number,0)=coalesce(v_round,0)
    and coalesce(effective_from_race_id,'00000000-0000-0000-0000-000000000000'::uuid)=coalesce(p_effective_from_race_id,'00000000-0000-0000-0000-000000000000'::uuid);
  ...
  if lower(p_role)='primary' then
    ...
    where sds.season_id=p_season_id and lower(sts.slot_label)=lower(v_team_name)
      and not exists (
        ...
        where dsa.driver_slot_id=sds.id and coalesce(dsa.effective_round_number,1)<=v_round
  ...
  update public.driver_season_assignments
  set team_id=case when lower(p_role)='inactive' then null else p_team_id end,
      league_team=case when lower(p_role)='inactive' then null else v_team_name end,
      is_primary=lower(p_role)='primary'
  ...
  if v_updated=0 then
    insert into public.driver_season_assignments (season_id,driver_id,team_id,league_team,is_primary,effective_from_race_id,effective_round_number)
    values (p_season_id,p_driver_id,case when lower(p_role)='inactive' then null else p_team_id end,
      case when lower(p_role)='inactive' then null else v_team_name end,lower(p_role)='primary',p_effective_from_race_id,v_round);
  end if;

  if lower(p_role)='primary' then
    insert into public.driver_slot_assignments (driver_slot_id,season_id,participant_driver_id,points_owner_driver_id,participation_mode,effective_from_race_id,effective_round_number)
    values (v_driver_slot_id,p_season_id,p_driver_id,p_driver_id,'player',p_effective_from_race_id,v_round)
    returning id into v_slot_assignment_id;
  end if;

  return jsonb_build_object('ok',true,'season_id',p_season_id,'driver_id',p_driver_id,'role',lower(p_role),'team_id',case when lower(p_role)='inactive' then null else p_team_id end,'effective_round_number',v_round,'slot_assignment_id',v_slot_assignment_id,'updated_existing_effective_assignment',v_updated>0);
end;
$$;
