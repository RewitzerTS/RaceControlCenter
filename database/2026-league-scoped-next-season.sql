...
begin
  ...
  values(p_league_id,v_slug,'Saison '||v_next,true,coalesce(nullif(trim(p_game_key),''),'f1_25'),coalesce(nullif(trim(p_game_label),''),'F1 25'))
  returning * into v_season;
  return jsonb_build_object('ok',true,'id',v_season.id,'slug',v_season.slug,'name',v_season.name,'league_id',v_season.league_id,'game_key',v_season.game_key,'game_label',v_season.game_label);
end;
$$;
